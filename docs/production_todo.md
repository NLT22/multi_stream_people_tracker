...
### Build checklist
- [x] **Incremental MTMC clustering core** — DONE: `src/mtmc/` (`IncrementalMTMC` + `Tracklet` schema + `run_incremental` sim harness). Within-batch agglomerative (cannot-link same-sensor) → Hungarian match to persistent anchor banks (eq.1 cost) → spawn/update/TTL-age. Validated on `64pm_office_0`: micro-batch **0.60** (batch=300f/20s, assign_thr 0.35–0.45) vs offline 0.836 — between online and offline, as predicted; the gap is per-tracklet-mean vs per-detection embedding (documented tradeoff). `python -m src.mtmc.run_incremental --pred-dir <export> --out-dir <out> --batch-frames 300 --assign-thr 0.40`.
