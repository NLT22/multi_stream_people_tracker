...
Thêm tầng **anchor-guided offline re-association** (§8, port từ AIC23 winner) **cải thiện vượt bậc ở industry**: 0.29/0.39 → **0.68/0.75**, retail → **0.45**, không regression scene nào — bằng cách cụm (cluster) lại danh tính offline trên toàn clip, thay cho online greedy matching (gán ID ngay, không sửa lại được).
...
| 64am_cafe_shop_0 | **0.980** | 0.928 | 0.051 | 0.990 |
| 63am_industry_safety_3 | **0.974** | 0.921 | 0.084 | 0.289 |
| 63am_office_3 | **0.963** | 0.899 | 0.111 | 0.541 |
| 63am_retail_3 | 0.648 | 0.580 | 4.98 | 0.221 |

**Phát hiện then chốt: occupancy density CHÍNH XÁC (CC 0.96–0.98) NGAY CẢ khi IDF1 thấp** — vd industry CC **0.974** dù IDF1 chỉ 0.289. Lý do: heatmap occupancy bám **detection** (mạnh), không bám **identity**. ⇒ analytics occupancy dùng được ở mọi môi trường kể cả nơi tracking-định-danh thất bại. retail thấp (CC 0.648) vì detector over-clean (recall 38–65% → thiếu người). *(Bảng trên là độ chính xác mật độ — không kèm ảnh foot heatmap; ảnh hiển thị dùng visitor heatmap ở §6.1.)*
...
*Visitor heatmap trải rộng theo đường đi của người (footfall/coverage), khác với occupancy/dwell vốn tập trung ở chỗ ngồi — đúng bản chất phân tích lượng khách.*

---

## 7. ĐỌC KẾT QUẢ (nút thắt mỗi scene)
Global IDF1 mỗi scene bị chặn bởi **1–2 camera yếu nhất**; loại nút thắt khác nhau:
- **cafe_shop**: mọi cam ~99%, 0 switch → 0.99 (đồng phục/ngoại hình phân biệt rõ, ít di chuyển).
...
## 8. ANCHOR-GUIDED OFFLINE RE-ASSOCIATION (cải thiện industry - tham khảo từ [bài báo](../docs/anchor-guided.pdf))

Thêm 2 tầng hậu xử lý **offline** từ AIC23 Track1 winner (CVPRW2023, IDF1 95.36;
`reference/AIC23_Track1_UWIPL_ETRI`) → `src/eval/offline_anchor.py`. Chạy trên
export `--pred-dir` có sẵn (KHÔNG chạy lại pipeline). Ý tưởng cốt lõi: pipeline
hiện tại gán Global ID kiểu **online greedy matching** (gán ID ngay từ frame đầu,
không sửa lại được); tầng offline **cluster lại danh tính trên toàn bộ clip** nên
sửa được các va chạm look-alike mà online không thể.

**Cách hoạt động** (mức tracklet — vì single-cam NvDCF đã mạnh, MOTA ~90%):
1. **Anchor clustering**: gom mean-embedding mỗi tracklet bằng Agglomerative thành
   đúng `k` danh tính (dùng thông tin toàn clip).
2. **auto-k (không cần GT)**: `k = max(dendrogram-gap, concurrency-floor)`, với
   concurrency-floor = phân vị 95 số người **đồng thời** trong 1 camera (đếm
   per-frame từ BEV). Cả 8 scene đều rơi đúng/sát k thật = 7.
3. **Fragmentation gate** (mặc định `1.25`): chỉ cụm lại khi online **phân mảnh**
   (online_ids/k ≥ gate); scene đã sạch (cafe/lobby/office_3) giữ nguyên →
   **không regression**.

| Scene | Baseline (online) | **Anchor (offline)** | Δ | quyết định (frag) |
|---|---|---|---|---|
| 63am_industry_safety_0 | 0.385 | **0.754** | **+0.369** | RELABEL (1.43) |
| 63am_industry_safety_3 | 0.289 | **0.703** | **+0.414** | RELABEL (1.86) |
| 63am_retail_3 | 0.221 | **0.449** | **+0.228** | RELABEL (1.43) |
| 63am_office_0 | 0.614 | **0.661** | +0.047 | RELABEL (1.43) |
| 63am_office_3 | 0.541 | 0.541 | 0 | KEEP (1.00) |
| 63am_lobby_3 | 0.871 | 0.871 | 0 | KEEP (1.14) |
| 64am_cafe_shop_0 | 0.990 | 0.990 | 0 | KEEP (1.00) |
| 64am_cafe_shop_3 | 0.888 | 0.888 | 0 | KEEP (1.14) |

- **industry cải thiện vượt bậc**: gần như gấp đôi/gấp ba (0.29→0.68, 0.39→0.75) — đây chính
  là nút thắt ID-switch look-alike ở §7, online không sửa được nhưng offline thì có.
- **retail tăng gấp đôi** (0.22→0.45) dù detector vẫn over-clean (FN lớn vẫn chặn trần).
- **office_3 KEEP**: nút thắt là **localization** (không phải định danh) → gate tự
  bỏ qua, không động vào — đúng (re-cluster không sửa được lỗi định vị).
- **STCRA (tách bằng hình học)**: đã port nhưng **mặc định TẮT** — khi cụm
  appearance đã sạch nó gây over-split (industry 0.754→0.727). Bật bằng `--stcra`.
- **Lưu ý gate**: ở gate `1.1` (mạnh tay), lobby_3 lên 0.871→0.932 (+0.06) NHƯNG
  cafe_shop_3 tụt 0.888→0.798 (−0.09) — cùng frag 1.14 không phân biệt được. Mặc
  định `1.25` (an toàn, ưu tiên không regression). Chi tiết: `anchor-guided-implement.md`.

### 8.2 Tối ưu anchor-guided (ablation)
Thử 3 hướng tối ưu từ paper; chỉ 1 hướng giúp, giữ lại:
- **auto-k ưu tiên concurrency-floor (ĐÃ ÁP DỤNG, +)**: dendrogram-gap ước lượng k
  bị **overshoot** ở scene look-alike (industry_3: gap=9 vs k thật=7 → over-cluster).
  Concurrency-floor (đỉnh số người đồng thời/1 camera) là tín hiệu đếm danh tính
  tin cậy, đúng trên mọi scene đã test → dùng làm chính, gap chỉ là fallback.
  **industry_3 0.678 → 0.703 (+0.025)**, các scene khác không đổi (floor vốn đã là max).
- **cannot-link constrained clustering (THỬ, BỎ)**: ép 2 tracklet cùng-camera đồng
  hiện ≥N frame là 2 người khác nhau. Với tracker phân mảnh của ta, greedy bị kẹt →
  over-split (industry_0 → 15 cụm, 0.754→0.508). `--cannot-link` (mặc định TẮT).
- **cosine+average linkage (THỬ, BỎ)**: như công thức cost của paper; hơi kém ward
  trên dữ liệu của ta (industry_0 0.754→0.740). Giữ ward.

## 9. KẾT LUẬN
- Làm sạch nhãn YOLO + clean-label ReID = **đòn bẩy lớn nhất** ở tầng online; đưa cafe/lobby (≥5') vượt 0.8.
- **Trần industry KHÔNG phải trần thật** — đó là giới hạn của **online greedy matching**. Tầng **anchor-guided offline** (§8) vượt hẳn lên: industry 0.29/0.39 → **0.75/0.70**, retail → **0.45**, không regression scene nào.
- office vẫn là **trần localization** (anchor không sửa được — cần fix calibration/detector trên cam yếu).
- retail cần fix riêng (conservative re-clean detector) — đang hoãn; anchor đã gấp đôi IDF1 nhưng FN detector vẫn chặn trần.
