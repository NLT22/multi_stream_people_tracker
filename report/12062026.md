...
### 1.2 MMPTracking_short ÍT bị ảnh hưởng
...
### 1.3 MMPTracking_10minute bị OVERFIT
...
### 1.4 Hướng đang xử lý: Đã gán nhãn lại từng scene, vì chỉ demo nên không sử dụng train val set nữa mà eval tương tự MMPTracking_short
...
Group heatmap (dwell, GT) — 4 camera lobby_0: phủ TOÀN THÂN / thời gian lưu lại (đậm hơn ở
nơi người ở lâu, vùng rộng hơn `foot`):

![lobby_0 dwell group](img/12062026/lobby_0_dwell_gt_group.png)
...
## Tham khảo
- **nvdsanalytics** (DeepStream plugin — ROI occupancy / line-crossing / overcrowding; xác nhận
  không có heatmap native, chỉ đếm theo vùng):
  https://docs.nvidia.com/metropolis/deepstream/dev-guide/text/DS_plugin_gst-nvdsanalytics.html
- **Gán Global-ID offline / near-realtime + lưu trữ** — Metropolis Multi-Camera Tracking
  microservice (merge IDs thành global ID, lưu DB + Elasticsearch để query-by-ID lâu dài):
  https://docs.nvidia.com/mms/text/MDX_Multi_Camera_Tracking_MS_Overview.html
- **RTLS** (real-time location, cập nhật vị trí global ID theo thời gian):
  https://docs.nvidia.com/mms/text/MDX_Real_Time_Location_System_App.html
- **Offline global-ID bằng spatio-temporal clustering** (AI City — anchor-guided clustering +
  ID re-assignment, gán ID toàn cục sau khi có cả chuỗi):
  https://arxiv.org/pdf/2304.09471
