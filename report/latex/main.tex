...
NvDCF hỗ trợ hai chế độ: Legacy DCF (\texttt{visualTrackerType:1}) và VPI DCF (\texttt{visualTrackerType:2}). Qua thực nghiệm trong giai đoạn lựa chọn tracker (chi tiết ở Phần~\ref{sec:throughput}), Legacy DCF cho thông lượng $\sim$18 FPS/camera ở 20 camera (khi đo chưa bao gồm SGIE), trong khi VPI DCF chỉ đạt $\sim$5 FPS/camera do chi phí khởi tạo cao hơn. Dự án sử dụng Legacy DCF. Pipeline production đầy đủ đạt $\sim$15 FPS/camera (chi tiết ở Phần~\ref{sec:throughput}).
...
Kiểm thử được thực hiện trên tập validation \textit{session 64pm} của MMPTrack gồm 24 scene thuộc 5 môi trường. Mỗi scene bao gồm 3--5 camera. Tất cả người trong tập validation là \textit{unseen subjects} --- không xuất hiện trong tập train. Đây là chế độ kiểm thử nghiêm ngặt nhất và phản ánh thực tế triển khai.
...
Bảng~\ref{tab:results} trình bày kết quả Global IDF1 của hệ thống trên toàn bộ tập validation MMPTrack --- 24 scene thuộc 5 môi trường --- so sánh với phương pháp baseline DMCT-Ext-TD \cite{Han2022}. Đây là lần đầu tiên hệ thống được đánh giá trên đầy đủ 24 scene thay vì chỉ 1 scene đại diện mỗi môi trường.
...
Trên toàn tập 24 scene, hệ thống đạt Global IDF1 trung bình \textbf{0,774} --- vượt baseline \cite{Han2022} 3,3 điểm phần trăm. Bốn trong năm môi trường (lobby, office, industry, café) đều vượt ngưỡng mục tiêu 0,8. Retail là môi trường duy nhất không đạt ngưỡng này, với IDF1 trung bình 0,616 trên 8 scene.
...
\textbf{Lobby (0,893, 4 scene):} Môi trường đạt kết quả cao nhất. Người di chuyển theo lối đi rõ ràng, camera được đặt ở góc tốt, và trang phục người dùng đa dạng giúp phân biệt. Kết quả ổn định qua cả 4 scene (0,872--0,906).

\textbf{Office (0,878, 3 scene):} Kết quả cao và đồng đều. Môi trường văn phòng có điều kiện ánh sáng tốt, mật độ người thấp, và người dùng có trang phục văn phòng đủ đa dạng để phân biệt.

\textbf{Industry (0,829, 5 scene):} Kết quả đạt ngưỡng mục tiêu, vượt kỳ vọng ban đầu. Thách thức chính là đồng phục lao động làm mô hình ReID khó phân biệt, nhưng Buffered ID với cửa sổ 20 giây giúp tích lũy đủ bằng chứng để gán đúng định danh. Biến thiên giữa các scene lớn hơn (0,711--0,877), cho thấy một số scene industry có điều kiện khó hơn.

\textbf{Café (0,823, 4 scene):} Kết quả tốt nhưng có biến thiên đáng kể giữa các scene (0,743--0,902). Hai scene cuối (café\_2, café\_3) có IDF1 thấp hơn, nhiều khả năng do góc camera hoặc mật độ người khác với café\_0.

\textbf{Retail (0,616, 8 scene):} Môi trường yếu nhất với biến thiên cao (0,427--0,675). Nguyên nhân chính: (1) kệ hàng cao gây che khuất, làm detector bỏ sót nhiều detection; (2) nhãn YOLO gốc có 39--40\% phantom annotations sau khi dọn sạch --- cho thấy có nhiều người thực sự bị che khuất hoàn toàn; (3) với 6 camera/scene (nhiều hơn các môi trường khác), không gian bán lẻ phức tạp và camera đặt gần góc khuất nhiều hơn. Retail\_1 (0,427) là scene khó nhất trong toàn tập val. Trần che khuất này là giới hạn vật lý của camera CCTV thông thường, không thể khắc phục hoàn toàn bằng cải thiện thuật toán.
...
\subsubsection{Năng lực hệ thống khi loại trừ môi trường Retail}
\label{sec:no_retail}

Vì Retail là môi trường duy nhất không đạt ngưỡng --- với nguyên nhân vật lý (che khuất nặng do kệ hàng) chứ không phải hạn chế thuật toán --- Bảng~\ref{tab:no_retail} trình bày kết quả trên 16 scene của 4 môi trường còn lại để định lượng năng lực thực của hệ thống khi không bị Retail kéo xuống.

\begin{table}[H]
    \centering
    \caption[Kết quả khi loại trừ môi trường Retail]{\bfseries\fontsize{12pt}{0pt}\selectfont Global IDF1 trên 16 scene của 4 môi trường (loại trừ Retail).}
    \begin{tabular}{|l|c|c|}
    \hline
    \bfseries Môi trường & \bfseries Số scene & \bfseries Global IDF1 \\
    \hline
    Lobby    & 4 & 0,893 \\
    Office   & 3 & 0,878 \\
    Industry & 5 & 0,829 \\
    Café     & 4 & 0,823 \\
    \hline
    \bfseries Trung bình & \bfseries 16 & \bfseries 0,853 \\
    \hline
    \end{tabular}
    \label{tab:no_retail}
\end{table}

Loại trừ Retail, điểm trung bình tăng từ \textbf{0,774 lên 0,853} (+0,079) và \textbf{cả bốn môi trường đều vượt ngưỡng 0,8}. Con số này định lượng rõ mức ảnh hưởng cục bộ của riêng Retail lên kết quả toàn tập: đây là vấn đề của một môi trường (lỗi phát hiện do che khuất), không phải hạn chế của toàn hệ thống.
...
Pipeline đầy đủ (YOLO + NvDCF + Swin SGIE + CrossCameraGalleryProbe) với preset mặc định (\texttt{reid0}, \texttt{maxTargetsPerStream=40}) đạt \textbf{$\sim$15 FPS/camera} ở 20 camera đồng thời (3{,}4 GB VRAM) --- vượt xa mục tiêu 10 FPS/cam. Ảnh hưởng của \texttt{maxTargetsPerStream} lên thông lượng được khảo sát chi tiết ở Bảng~\ref{tab:maxtargets}.
...
Giá trị mặc định production được chọn là \textbf{\texttt{maxTargetsPerStream = 40}}: đủ biên an toàn cho scene đông người (mỗi camera MMP có tối đa $\sim$7 người, nhưng NvDCF còn giữ \textit{shadow track} và \textit{tentative track} chiếm thêm ô) trong khi vẫn đạt $\sim$15 FPS/cam và chỉ 3,4 GB VRAM, để lại $\geq$12 GB dự phòng trên RTX 5060 Ti 16 GB.
...
Để đánh giá ảnh hưởng của kích thước cửa sổ Buffered ID trên toàn tập validation, một thí nghiệm ablation được thực hiện trên đầy đủ 24 scene: pipeline chạy với chunk 100 khung, sau đó thuật toán anchor-guided được áp dụng lại với 4 kích thước cửa sổ (100--400 khung) trên cùng tập embedding. Kết quả Global IDF1 trung bình theo môi trường trình bày trong Bảng~\ref{tab:window_ablation}. Thí nghiệm này thay thế đánh giá sơ bộ trước đó (chỉ 5 scene đại diện) và là đánh giá chính thức trên toàn tập.
...
Kết quả trên toàn 24 scene cho thấy sự khác biệt giữa các kích thước cửa sổ rất nhỏ (khoảng cách tối đa 0,005 điểm). Cửa sổ 300 khung ($\sim$30 giây) đạt trung bình cao nhất (0,766), nhỉnh hơn không đáng kể so với 100 khung (0,764). Kết quả đáng chú ý là \textbf{Industry ưa cửa sổ ngắn hơn} (100 khung tốt nhất, 0,845), trái với nhận định từ thí nghiệm 5 scene trước đó --- lý do là cửa sổ ngắn giảm thiểu sự nhầm lẫn khi nhiều người đồng phục xuất hiện rồi biến mất trong cùng khung thời gian. Retail gần như không nhạy cảm với kích thước cửa sổ (0,603--0,609), xác nhận giới hạn của môi trường này nằm ở chất lượng embedding, không phải tham số hậu xử lý. Vì chênh lệch giữa các kích thước nằm trong dải nhiễu, cửa sổ 200 khung ($\sim$20 giây) --- giá trị mặc định --- không phải là tốt nhất tuyệt đối (300 khung nhỉnh hơn 0,005 điểm) nhưng vẫn là lựa chọn hợp lý.

Nếu chọn cửa sổ tối ưu riêng cho từng môi trường (Industry~=~100f, Office~=~200f, Café/Retail~=~300f, Lobby~=~100f) và tính kết quả oracle tốt nhất có thể đạt được trên từng scene, trần trên là \textbf{0,781} --- vẫn dưới ngưỡng 0,800 đặt ra. Điều này xác nhận rào cản chính không nằm ở kích thước cửa sổ mà ở chất lượng embedding ReID trong môi trường retail.

\subsection{Nghiên cứu tối ưu hóa thông lượng}

Để tìm cách tăng FPS mà không giảm IDF1, hệ thống được thử nghiệm với lượng tử hóa INT8 cho cả detector YOLO11n và mạng Swin ReID. Các thí nghiệm trong phần này được thực hiện với \texttt{maxTargetsPerStream=220} (cấu hình tại thời điểm đo), nên mốc FPS gốc ($\sim$10,7 FPS/cam) thấp hơn cấu hình mặc định hiện tại (40); kết luận về \textit{mức thay đổi} FPS giữa các thử nghiệm không bị ảnh hưởng bởi giá trị này. Bảng~\ref{tab:speedup} tổng hợp kết quả:
...
\begin{itemize}
    \item \textbf{Thông lượng:} $\sim$15 FPS/camera tại 20 camera đồng thời, preset reid0 (\texttt{maxTargetsPerStream = 40}, 3,4 GB VRAM) --- vượt xa mục tiêu 10 FPS/cam.
    \item \textbf{Độ chính xác:} Đánh giá trung thực trên toàn bộ 24 scene của tập validation MMPTrack, Global IDF1 trung bình đạt 0,774 với người unseen --- vượt baseline công bố (0,741). Bốn trong năm môi trường (lobby, office, industry, café) đều vượt ngưỡng mục tiêu 0,8; nếu loại môi trường retail, trung bình 4 môi trường còn lại đạt 0,853. Riêng retail (che khuất nặng) chưa đạt ngưỡng, kéo điểm trung bình toàn tập xuống dưới 0,8.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Môi trường retail:} Global IDF1 chỉ đạt 0,616 (trung bình 8 scene), thấp hơn đáng kể so với các môi trường khác. Nút thắt là khâu \textit{phát hiện người} dưới che khuất do kệ hàng (detector bỏ sót nhiều detection), không phải khâu ghép định danh --- đây là giới hạn vật lý của camera CCTV thông thường, không thể giải quyết hoàn toàn bằng thuật toán.
...
\begin{itemize}
    ...
    \item \textbf{Tích hợp mô hình BEV:} Nghiên cứu khả năng tích hợp MCBLT \cite{Wang2025} hoặc phương pháp BEV tương tự vào pipeline, hoặc sử dụng như post-processing layer để cải thiện định danh trong các scene khó.
