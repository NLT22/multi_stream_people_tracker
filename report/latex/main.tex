...
\begin{itemize}
    \item \textbf{Chương 2 --- Cơ sở lý thuyết:} Định nghĩa bài toán MTMC, các độ đo đánh giá, hai bộ dữ liệu MMPTrack và MTMC (kèm so sánh đặc điểm/hiệu chỉnh camera), các thành phần mô hình (YOLO11n, NvDCF, Swin ReID, DeepStream), và tổng quan các nghiên cứu liên quan.
    ...
    \item \textbf{Chương 4 --- Thí nghiệm và kết quả:} Thiết lập thực nghiệm, kết quả theo dõi theo từng môi trường, phân tích throughput, độ chính xác heatmap, so sánh kiến trúc ReID, và trực quan hoá pipeline trên MTMC.
...
MTMC khác MMPTrack ở ba điểm quyết định thiết kế thuật toán (Bảng~\ref{tab:mmp_vs_mtmc}): (1) camera rời rạc (disjoint FOV) --- mỗi người thường chỉ xuất hiện ở một camera tại một thời điểm, nên bài toán cốt lõi là bàn giao (hand-off) danh tính khi người di chuyển giữa các vùng camera, ngược với FOV chồng lắp của MMP; (2) có \texttt{calibration.json} metric (ma trận nội $K$ + ngoại $[R|t]$ cho mỗi camera) mà MMP không có; (3) \texttt{object\_id} là định danh xuyên-camera thực và nhãn 2D là hộp nhìn thấy được (không chiếu amodal như MMP).
...
\begin{table}[H]
    ...
    FOV camera & chồng lắp (overlapping) & rời rạc (disjoint) \\
    Hiệu chỉnh & không (chỉ homography sàn) & metric $K$ + $[R|t]$ \\
    Nhãn 2D & chiếu amodal (cả người bị che) & hộp nhìn thấy được \\
...
Khác biệt về hình học đặt camera (Bảng~\ref{tab:cam_geometry}, tính từ ma trận ngoại: độ cao là thành phần $z$ của tâm camera $C=-R^{\top}t$; góc chúi là góc của trục quang dưới phương ngang) giải thích trực tiếp chênh lệch chất lượng phát hiện giữa hai bộ dữ liệu. Camera MMP chúi dốc (trung bình 41$^\circ$) ở độ cao $\sim$4 m trong phòng kín, nhìn gần như từ trên xuống nên người to, rõ, ít bị che và recall phát hiện cao. Camera kho MTMC Warehouse\_022 chúi nông hơn (trung bình 24$^\circ$) và đặt cao hơn ($\sim$6,8 m), nhìn gần như ngang dọc lối đi nên người ở xa nhỏ và bị kệ hàng che khuất nhiều, recall phát hiện thấp hơn. Góc nông cũng khiến các camera ít chồng lắp đồng thời, là lý do trên MTMC khâu hợp nhất ưu tiên vị trí hình học thay vì ngoại hình (Phần~\ref{sec:mtmc_results}).
...
\begin{itemize}
    ...
    \item \textbf{ROI editor} --- vẽ vùng/vạch trực tiếp trên khung hình và \textit{sinh ra cấu hình} \texttt{gst-nvdsanalytics} đúng định dạng;
...
Bảng baseline trong \cite{Han2022} không cung cấp phân tích per-environment, nên dấu ``---'' biểu thị không có số liệu so sánh trực tiếp ở các dòng tương ứng.

\subsubsection{Phân tích theo môi trường}

Lobby (0,906) và office (0,880) đạt cao nhất nhờ điều kiện thuận lợi: lối đi rõ ràng, ánh sáng tốt, mật độ người thấp và trang phục đa dạng giúp ReID phân biệt dễ dàng. Industry (0,847) vẫn đạt ngưỡng mục tiêu dù người mặc đồng phục lao động --- Buffered ID với cửa sổ 20 giây tích lũy đủ bằng chứng để gán đúng định danh. Café (0,839) cho kết quả tốt nhưng biến thiên đáng kể giữa các cảnh (0,733--0,931). Retail (0,661) là môi trường yếu nhất: dù huấn luyện lại detector trên nhãn sạch đã nâng kết quả từ 0,616, kệ hàng cao vẫn che khuất khiến detector bỏ sót nhiều người thực (recall giảm) --- giới hạn vật lý không khắc phục được bằng thuật toán, cũng phản ánh qua việc $\sim$42\% nhãn YOLO gốc của retail là phantom hoặc bị che khuất.
...
Mục tiêu sản phẩm là 20 camera ở 1920$\times$1080 (độ phân giải camera production thực), trong khi tập MMP chỉ 640$\times$360. Để kiểm chứng, cấu trúc pipeline MMP (\texttt{maxTargetsPerStream=40}, \texttt{reidType:0}) được chạy trên 20 luồng video 1080p của MTMC. Phép đo dùng detector huấn luyện trên dữ liệu kho để phản ánh đúng tải thực: nếu dùng detector MMP (huấn luyện trong nhà) trên video kho, detector không phát hiện được người nên tầng SGIE ReID --- vốn là nút thắt thực sự của pipeline --- không hoạt động và cho con số FPS không phản ánh đúng tải. Kết quả với detector phù hợp được trình bày trong Bảng~\ref{tab:throughput_1080p}.
...
\begin{table}[H]
    ...
    \hline
    Dữ liệu MMP (tham chiếu) & 640 & 40 & 3,4 GB & $\sim$15 \\
...
Pipeline MMP (maxTargets=40, input detector 640) trên \textbf{20 camera 1080p thật đạt 3,83 GB / $\sim$16 FPS/cam} --- gần như trùng với số liệu trên dữ liệu MMP gốc (3,4 GB / $\sim$15 FPS/cam), xác nhận pipeline \textbf{giữ vững mục tiêu 20 camera ngay ở độ phân giải 1080p}. Nâng input detector lên 960 giảm còn $\sim$12 FPS/cam; phần VRAM tăng thêm (ví dụ 6,27 GB khi đặt \texttt{maxTargets=100} cho cấu hình kho) là do \texttt{maxTargetsPerStream}, không phải do độ phân giải.
