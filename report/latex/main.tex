...
Pipeline đầy đủ (YOLO + NvDCF + Swin SGIE + CrossCameraGalleryProbe) với preset mặc định (\texttt{reid0}) đạt \textbf{$\sim$15 FPS/camera} ở 20 camera đồng thời khi đặt \texttt{maxTargetsPerStream=40} (3{,}4 GB VRAM), hoặc \textbf{$\sim$16{,}5 FPS/camera} ở \texttt{maxTargetsPerStream=10} (2{,}4 GB VRAM) --- vượt xa mục tiêu 10 FPS. Con số $\sim$10{,}6 FPS/cam (9{,}4 GB) báo cáo trước đây tương ứng với cấu hình cũ \texttt{maxTargetsPerStream=220}; xem khảo sát chi tiết ở Bảng~\ref{tab:maxtargets}.
...
Thử nghiệm quantization INT8 không mang lại cải thiện FPS có ý nghĩa (dưới 0{,}5 FPS/cam) trong khi chỉ tiết kiệm $\sim$0{,}37 GB VRAM, nên không được áp dụng trong pipeline production. Đòn bẩy quan trọng hơn nhiều đối với cả VRAM lẫn FPS là tham số \texttt{maxTargetsPerStream}, được khảo sát chi tiết dưới đây.
...
Kết quả cho thấy \texttt{maxTargetsPerStream} là đòn bẩy chi phối: giảm từ 220 xuống 10 giúp \textbf{tăng FPS 55\%} (10,62 $\to$ 16,50 FPS/cam) và \textbf{giảm VRAM 74\%} (9,16 $\to$ 2,40 GB). Con số ``$\sim$10,6 FPS/cam và $\sim$9,4 GB VRAM'' từng được báo cáo cho preset reid0 chính là trường hợp \texttt{maxTargetsPerStream=220}; sau khi hạ xuống giá trị hợp lý (MMP không bao giờ cần quá $\sim$15 người/camera nên 40 là dư), pipeline đạt $\sim$15 FPS/cam ở chỉ 3,4 GB VRAM. Giá trị mặc định production được đặt ở khoảng 10--40 tùy yêu cầu: 10 cho thông lượng/VRAM tối ưu, 40 nếu cần biên an toàn cho cảnh đông người. Pipeline để lại $\geq$12 GB dự phòng trên RTX 5060 Ti 16 GB ở các cấu hình này.
...
Để xác nhận kích thước cửa sổ tối ưu trên toàn tập validation, một thí nghiệm ablation được thực hiện trên đầy đủ 24 cảnh: pipeline chạy với chunk 100 khung, sau đó thuật toán anchor-guided được áp dụng lại với 4 kích thước cửa sổ (100--400 khung) trên cùng tập embedding. Kết quả Global IDF1 trung bình theo môi trường trình bày trong Bảng~\ref{tab:window_ablation}. Thí nghiệm trước đó (5 cảnh đại diện) cho kết quả tương tự nhưng có sai lệch nhỏ do mẫu nhỏ; kết quả 24 cảnh là đánh giá chính thức.

\begin{table}[H]
    \centering
    \caption[Khảo sát kích thước cửa sổ Buffered ID --- toàn tập 24 cảnh]{\bfseries\fontsize{12pt}{0pt}\selectfont Global IDF1 trung bình theo kích thước cửa sổ Buffered ID (24 cảnh val).}
    \begin{tabular}{|l|c|c|c|c|}
    \hline
    \bfseries Môi trường & \bfseries 100f & \bfseries 200f & \bfseries \underline{300f} & \bfseries 400f \\
    \hline
    Industry  & \textbf{0,845} & 0,810 & 0,822 & 0,831 \\
    Lobby     & \textbf{0,894} & 0,883 & 0,890 & 0,884 \\
    Office    & 0,858 & \textbf{0,874} & 0,867 & 0,867 \\
    Café      & 0,786 & 0,809 & \textbf{0,813} & 0,785 \\
    Retail    & 0,604 & 0,603 & \textbf{0,609} & 0,604 \\
    \hline
    \textbf{Trung bình} & 0,764 & 0,761 & \textbf{0,766} & 0,761 \\
    \hline
    \end{tabular}
    \label{tab:window_ablation}
\end{table}

Kết quả trên toàn 24 cảnh cho thấy sự khác biệt giữa các kích thước cửa sổ rất nhỏ (khoảng cách tối đa 0,005 điểm). Cửa sổ 300 khung ($\sim$30 giây) đạt trung bình cao nhất (0,766), nhỉnh hơn không đáng kể so với 100 khung (0,764). Kết quả đáng chú ý là \textbf{Industry ưa cửa sổ ngắn hơn} (100 khung tốt nhất, 0,845), trái với nhận định từ thí nghiệm 5 cảnh trước đó --- lý do là cửa sổ ngắn giảm thiểu sự nhầm lẫn khi nhiều người đồng phục xuất hiện rồi biến mất trong cùng khung thời gian. Retail gần như không nhạy cảm với kích thước cửa sổ (0,603--0,609), xác nhận giới hạn của môi trường này nằm ở chất lượng embedding, không phải tham số hậu xử lý.

Nếu chọn cửa sổ tối ưu riêng cho từng môi trường (Industry~=~100f, Office~=~200f, Café/Retail~=~300f, Lobby~=~100f) và tính kết quả oracle tốt nhất có thể đạt được trên từng cảnh, trần trên là \textbf{0,781} --- vẫn dưới ngưỡng 0,800 đặt ra. Điều này xác nhận rào cản chính không nằm ở kích thước cửa sổ mà ở chất lượng embedding ReID trong môi trường retail.
