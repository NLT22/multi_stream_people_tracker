...
\begin{enumerate}
    ...
    \item \textbf{Buffered ID dựa trên anchor-guided clustering:} Triển khai thuật toán anchor-guided từ \cite{Huang2023} thành chế độ cửa sổ trượt 30 giây trong pipeline thời gian thực, đạt 97--98\% chất lượng của xử lý offline.
...
\begin{itemize}
    \item \textbf{Chương 2 --- Cơ sở lý thuyết:} Định nghĩa bài toán MTMC, các độ đo đánh giá, bộ dữ liệu MMPTrack, các thành phần mô hình (YOLO11n, NvDCF, Swin ReID, DeepStream), và tổng quan các công trình liên quan.
...
NvDCF hỗ trợ hai chế độ: Legacy DCF (\texttt{visualTrackerType:1}) và VPI DCF (\texttt{visualTrackerType:2}). Qua thực nghiệm trong giai đoạn lựa chọn tracker (chi tiết ở Phần~\ref{sec:throughput}), Legacy DCF cho thông lượng $\sim$18 FPS/camera ở 20 camera (khi đo chưa bao gồm SGIE), trong khi VPI DCF chỉ đạt $\sim$5 FPS/camera do chi phí khởi tạo cao hơn. Dự án sử dụng Legacy DCF. Pipeline sản xuất đầy đủ đạt $\sim$10{,}6 FPS/camera (chi tiết ở Phần~\ref{sec:throughput}).
...
Bảng~\ref{tab:results} trình bày kết quả Global IDF1 của hệ thống trên 5 môi trường kiểm thử. Kết quả so sánh hai chế độ: Buffered ID (chế độ sản xuất) và phương pháp baseline DMCT-Ext-TD \cite{Han2022}.
...
\subsubsection{Thông lượng pipeline sản xuất}
...
Pipeline sản xuất sử dụng $\sim$9{,}4 GB VRAM trên RTX 5060 Ti 16 GB, để lại 6{,}6 GB dự phòng. Thử nghiệm quantization INT8 không mang lại cải thiện FPS có ý nghĩa (dưới 0{,}5 FPS/cam) trong khi chỉ tiết kiệm $\sim$0{,}37 GB VRAM, nên không được áp dụng trong pipeline sản xuất.
...
\subsubsection{Lượng tử hóa INT8}
...
\subsubsection{Thử nghiệm Sidecar ReID}

Một kiến trúc thay thế được cài đặt: đưa phần suy luận Swin ra khỏi đồ thị GStreamer vào một tiến trình nền (\textit{sidecar}), tách khỏi đường tới hạn. Kiến trúc gồm \texttt{MetaCaptureOp} $\to$ \texttt{VideoReaderPool} $\to$ \texttt{ReIDWorker} (ONNX CUDAExecutionProvider). Kết quả: sidecar không cải thiện FPS (thậm chí chậm hơn nhẹ do chi phí giải mã video bằng CPU thêm vào). SGIE không phải nút thắt --- phần detect + track + tiler đã chiếm gần toàn bộ thời gian ($\sim$10{,}6 FPS với preset reid0 không có SGIE cũng gần tương đương). Kiến trúc sidecar vẫn hữu ích để thử nhanh một mô hình ReID mới mà không cần biên dịch lại engine TensorRT.

\subsection{So sánh kiến trúc mô hình ReID}

Để xác nhận lựa chọn Swin-Tiny là kiến trúc ReID tốt nhất cho hệ thống, OSNet-x1.0 (khởi đầu từ pretrain Market-1501 qua torchreid) được huấn luyện trên cùng tập MMP và so sánh:
...
Hai đóng góp kỹ thuật quan trọng giúp đạt được các kết quả trên là: (1) quy trình làm sạch nhãn loại bỏ nhiễu đáng kể trong dữ liệu MMPTrack; (2) triển khai Buffered ID dựa trên anchor-guided clustering \cite{Huang2023} trong cửa sổ trượt 30 giây, giúp cải thiện Global IDF1 từ 0,289 lên 0,754 trong môi trường khó (industry đồng phục).
